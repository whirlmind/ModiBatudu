...
ModiBatudu_Setup.py : This can be found in the folder for source-code. This is accessed while py2exe tries to convert ModiBatudu.py into ModiBatudu.exe.

If both these are present, you would typically run:
 "C:\Folder_goes_here\ModiBatudu_Setup.py py2exe " at the command prompt to produce ModiBatudu.exe. 
 
 You will need to this only if you change the source code. You might want to notice, that as per this set up file, the destination folder for the EXE, the dependent DLL etc, will the the "dist" subfolder of the folder from which this setup script is run. 
